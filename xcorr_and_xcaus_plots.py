import pandas as pd
import numpy as np
from analysis.precomput_analysis_funcs import scale_then_pca_then_save, plot_variance_expl_plot, clustering_after_pca, tsne_after_pca, nmf_then_save
...
class xcorr_manager():
    def __init__(self):
        ...
        self.num_samples_hx = self.hp.analysis.agent_h.num_episodes  # number of generated samples to use
        self.num_samples_hx_grad = 400
        self.num_samples_env_h = 6000
        n_components_hx = 64
        n_components_env_h = 64
        self.ts_per_sample = 10
        self.direction_type = 'ica'
        self.num_ica_components = 16
        self.hx_projector = HiddenStateDimensionalityReducer(self.hp,
                                                         self.direction_type,
                                                         self.num_samples_hx,
        ...
        hx_precomp_data_path = os.path.join('analysis',
                                            'hx_analysis_precomp')
        self.env_h_precomp_path = os.path.join('analysis',
                                               'env_analysis_precomp')
        ...
        self.hx_pc_variances = np.load(
            f'{hx_precomp_data_path}/pc_loading_variances_{self.num_samples_hx}.npy') #TODO put the extra
        ...
        self.hx_std = np.load(hx_precomp_data_path + f'/hx_std_{self.num_samples_hx}.npy')
        # self.grad_projector = self.project_gradients_into_pc_space

        if self.direction_type == 'ica':
            ica_directions_path = os.path.join(hx_precomp_data_path,
            ...
            self.mix_mat = np.load(f"{hx_precomp_data_path}/ica_mixing_matrix_hx_{self.num_samples_hx}.npy")
        ...
        self.save_path = 'analysis/xcorr_matrices/'
    ...
    def plot_direction_xcausation_multi_timestep(self):
        ...
        for ep in range(0, self.num_samples_hx_grad):
            ...
            for direction_id in range(self.num_ica_components):
                grads_name = f'grad_hx_hx_direction_{direction_id}_{self.direction_type}'
                grads = np.load(os.path.join(self.generated_data_path,
                                          f'sample_{ep:05d}/{grads_name}.npy'))  # (T x hx)
            ...
            stacked_sample_grads = np.stack(sample_dir_grads, axis=-1) # forms the rows (by sticking columns together)
            samples_dir_grads.append(stacked_sample_grads) # So the columns are 'grads_wrt, the rows are 'grads_of'
        ...
        for ts in range(stacked_samples_grads.shape[0]):
            ...
            self.plot_heatmap(ts_slice, "xcaus_hx_t+%i" % int(ts), title_name,
                              labels=None, type_of_xcorr='causation', clim=ts_slice.max()/2)
    ...
    def plot_direction_xcorrs_multi_timestep(self):
        ...
        samples_data_env = []
        samples_data_hx = []
        env_pca_data = np.load(self.env_h_precomp_path + f'/pca_data_env_{self.num_samples_env_h}.npy')
        ...
        for ep in range(0, min(self.num_samples_hx, self.num_samples_env_h)):
            ...
            hx = np.load(os.path.join(self.generated_data_path,
        ...
        for k in np.arange(0, K+1, 1):
            ...
            if k == 0:
                ordered_dist_mat, res_order, res_linkage = self.compute_serial_matrix(
                    squareform(pdist(xcorrs_hx)),
                    'ward')
                self.plot_dendrogram(res_linkage,
                                     f'dendrogram_{self.direction_type}')

            print("Plotting for k=%i" % int(k))
            title_name = f"Cross-correlation matrix comparing t (y-axis) with t+{k} (x-axis)"

            self.plot_heatmap(xcorrs_hx, "xcorr_hx_t+%i" % int(k), title_name, res_order)
            self.plot_heatmap(xcorrs_hx, "unordered_xcorr_hx_t+%i" % int(k), title_name)

    def plot_heatmap(self, matrix, name, plot_title, labels=None, type_of_xcorr='causation', clim=1.):

        plt.rcParams["figure.figsize"] = (10, 10)
        if labels is None:
            plt.imshow(matrix, cmap='seismic_r',
                       interpolation='nearest')
            plt.xticks(ticks=np.arange(0, matrix.shape[0], 1.))
            plt.yticks(ticks=np.arange(0, matrix.shape[0], 1.))
        else:
            plt.imshow(matrix[:,labels], cmap='seismic_r',
                       interpolation='nearest')
            plt.xticks(ticks=np.arange(0, matrix.shape[0], 1.), labels=labels)
            plt.yticks(ticks=np.arange(0, matrix.shape[0], 1.))
        plt.colorbar()
        plt.clim(-clim, clim)
